...
class ICEDataIntegrator:
    def get_cenceweb_summary(self) -> dict:
        """Intenta obtener resumen de datos clave de https://apps.grupoice.com/CenceWeb/ y subpáginas. Fallback a simulado."""
        import re, datetime
        summary = {}
        try:
            # Demanda actual (CurvaDemanda.html)
            demanda = self.get_real_time_demand()
            summary["demanda_actual_mw"] = demanda.get("demanda_actual_mw")
            summary["demanda_hora"] = demanda.get("hora")
            # Generación real (GeneracionReal.html)
            url_gen = "https://apps.grupoice.com/CenceWeb/paginas/GeneracionReal.html"
            resp_gen = requests.get(url_gen, timeout=6)
            if resp_gen.status_code == 200:
                match = re.search(r"var\s+datosGeneracion\s*=\s*(\[.*?\]);", resp_gen.text, re.DOTALL)
                if match:
                    import ast
                    gen_data = ast.literal_eval(match.group(1))
                    if gen_data and isinstance(gen_data, list):
                        gen_actual = gen_data[-1][1]
                        summary["generacion_actual_mw"] = gen_actual
                        summary["generacion_hora"] = gen_data[-1][0]
                        summary["generacion_serie"] = gen_data
            # Reservas SEN (ReservasSEN.html)
            url_res = "https://apps.grupoice.com/CenceWeb/paginas/ReservasSEN.html"
            resp_res = requests.get(url_res, timeout=6)
            if resp_res.status_code == 200:
                match = re.search(r"var\s+datosReservas\s*=\s*(\[.*?\]);", resp_res.text, re.DOTALL)
                if match:
                    import ast
                    res_data = ast.literal_eval(match.group(1))
                    if res_data and isinstance(res_data, list):
                        res_actual = res_data[-1][1]
                        summary["reserva_actual_mw"] = res_actual
                        summary["reserva_hora"] = res_data[-1][0]
                        summary["reserva_serie"] = res_data
            summary["fuente"] = "CENCE ICE (scraping)"
            summary["timestamp"] = datetime.datetime.now().isoformat()
            return summary
        except Exception as e:
            print(f"[ERROR][get_cenceweb_summary]: {e}")
            import traceback
            traceback.print_exc()
        # Fallback simulado
        now = datetime.datetime.now()
        sim_series = [[(now.replace(hour=h, minute=0, second=0, microsecond=0)).strftime("%H:%M"), 1500+100*((h-18)%24>0 and (h-18)%24<3)] for h in range(0,24)]
        return {
            "fuente": "Simulado (histórico)",
            "demanda_actual_mw": sim_series[now.hour][1],
            "demanda_hora": sim_series[now.hour][0],
            "demanda_serie": sim_series,
            "generacion_actual_mw": sim_series[now.hour][1]-50,
            "generacion_hora": sim_series[now.hour][0],
            "generacion_serie": sim_series,
            "reserva_actual_mw": 200,
            "reserva_hora": sim_series[now.hour][0],
            "reserva_serie": [[t,200] for t,_ in sim_series],
            "timestamp": now.isoformat()
        }
    def get_real_time_demand(self) -> dict:
        """Intenta obtener demanda/generación real de la web de ICE. Si falla, retorna datos simulados."""
        import re
        try:
            url = "https://apps.grupoice.com/CenceWeb/paginas/CurvaDemanda.html"
            resp = requests.get(url, timeout=6)
            if resp.status_code == 200:
                # Buscar el bloque de datos en el HTML (JavaScript array)
                match = re.search(r"var\s+datosDemanda\s*=\s*(\[.*?\]);", resp.text, re.DOTALL)
                if match:
                    import ast
                    demanda_data = ast.literal_eval(match.group(1))
                    # Extraer último valor (demanda actual)
                    if demanda_data and isinstance(demanda_data, list):
                        demanda_actual = demanda_data[-1][1]
                        hora_actual = demanda_data[-1][0]
                        return {
                            "fuente": "ICE CenceWeb (scraping)",
                            "demanda_actual_mw": demanda_actual,
                            "hora": hora_actual,
                            "serie_completa": demanda_data
                        }
            # Si no hay datos, fallback
        except Exception as e:
            print(f"[ERROR][get_real_time_demand]: {e}")
            import traceback
            traceback.print_exc()
        # Fallback: datos simulados históricos
        import random, datetime
        now = datetime.datetime.now()
        simulated_series = [[(now.replace(hour=h, minute=0, second=0, microsecond=0)).strftime("%H:%M"), random.randint(1200, 2000)] for h in range(0, 24)]
        return {
            "fuente": "Simulado (histórico)",
            "demanda_actual_mw": simulated_series[now.hour][1],
            "hora": simulated_series[now.hour][0],
            "serie_completa": simulated_series
        }
    def collect_all_official_data(self) -> Dict:
        """Recolecta y combina datos de todas las fuentes oficiales para entrenamiento IA y dashboard. Incluye demanda/generación/reservas real si es posible."""
        combined_data = {}
        # Resumen CENCE ICE (scraping o simulado)
        combined_data["cenceweb_resumen"] = self.get_cenceweb_summary()
        # Endpoints ICE
        combined_data["ice_endpoints"] = self.real_time_endpoints
        # Fuentes adicionales de TRANSPARENCIA_DATOS_OFICIALES.md
        combined_data["cicr_tarifas_pdf"] = "https://cicr.com/wp-content/uploads/2023/05/Propuesta_Tarifaria_electrificacion.pdf"
        combined_data["aresep_cobertura"] = "Informes anuales ARESEP"
        combined_data["eia_benchmark"] = "https://www.eia.gov/electricity/"
        combined_data["ice_empresarial"] = "https://www.grupoice.com/wps/portal/ICE/electricidad/servicioss/empresariales"
        combined_data["ice_prepago"] = "https://www.grupoice.com/wps/portal/ICE/electricidad/electricidadprepago/electricidadprepago"
        combined_data["marco_legal"] = "https://pgrweb.go.cr/scij/Busqueda/Normativa/Normas/nrm_texto_completo.aspx?param1=NRTC&nValor1=1&nValor2=71712&nValor3=87131&strTipM=TC"
        # Datos tabulares y proyecciones del markdown pueden ser integrados aquí como referencia
    ...
    def __init__(self):
        ...
        self.real_time_endpoints = {
            ...
            "curva_demanda": "https://apps.grupoice.com/CenceWeb/paginas/CurvaDemanda.html",
            "generacion_real": "https://apps.grupoice.com/CenceWeb/paginas/GeneracionReal.html",
            "cenceweb_home": "https://apps.grupoice.com/CenceWeb/",
            "curva_demanda_comparativo": "https://apps.grupoice.com/CenceWeb/paginas/CurvaDemandaComparativo.html?anno=2025&mes=8",
            "reservas_sen": "https://apps.grupoice.com/CenceWeb/paginas/ReservasSEN.html?fecha=08/08/2025",
            "intercambios_regionales": "https://apps.grupoice.com/CenceWeb/paginas/IntercambiosRegionales.html"
        }
